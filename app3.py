...
# int: version_int, str: version_str, float: version_float, bool: is_active
version_int = 1
version_str = "1.0"
version_float = 1.0
is_active = True
...
# frozenset: Allowed file formats
allowed_formats = frozenset(["JSON", "CSV", "TXT"])

# dict: Psychological states
psych_states = {
...
def load_questions() -> list: # list variable type
...
# ---------------- HELPERS & VALIDATION (Conditionals & Loops) ----------------
def validate_name(name: str) -> bool:
    """Uses a WHILE loop and SET to validate the name."""
    if not name: 
        return False
        
    # WHILE LOOP for input validation (Removing accidental double spaces)
    while "  " in name:
        name = name.replace("  ", " ")
        
    # SET variable type for validation
...
def validate_student_id(sid: str) -> bool:
    """Uses a FOR loop to validate that ID contains only digits."""
    if not sid: 
        return False
...
def interpret_score(score: int) -> str:
    # Use of IF, ELIF, ELSE (Conditional statements)
